...
void SetFirebase()
{
    builder.Services.Configure<FirebaseConfig>(options => configuration.GetSection("FirebaseSettings").Bind(options));

    builder.Services.AddHttpClient<FirebaseSender>();
    builder.Services.AddHttpClient<ApnSender>();

    builder.Services.AddSingleton(provider =>
    {
        string configPath = builder.Configuration["FirebaseSettings:ServiceAccountFilePath"] ?? string.Empty;
        string jsonContent = File.ReadAllText(configPath);

        if (string.IsNullOrEmpty(jsonContent)) throw new Exception("No se encontró el archivo Json.");

        return new FirebaseSender(jsonContent, new HttpClient());
    });
